...
class FBP(nn.Module):
    def __init__(self, d_emb_1, d_emb_2, fbp_hid, fbp_k, dropout):
        ...
        self.fusion_pooling = nn.AvgPool1d(kernel_size=fbp_k)#2
        self.fbp_k = fbp_k
...
class MultiHeadAttention(nn.Module):
    def __init__(self, n_head, d_emb_q, d_emb_v, d_k=None, d_v=None, dropout=0.1):
        ...
        self.d_k = d_k if d_k is not None else d_emb_q #74
        self.d_v = d_v if d_v is not None else d_emb_v #35
        ...
        self.fc = nn.Linear(self.d_v, d_emb_q, bias=False)#35-74

        self.fbp = FBP(self.d_k, self.d_k, 32, 2, dropout)#统一到64维度
        ...
        #点击注意力
        self.attention = ScaledDotProductAttention(temperature=self.d_k ** 0.5, attn_dropout=dropout)#平方根8.602325267042627
        self.dropout = nn.Dropout(p=dropout)
        self.layer_norm = nn.LayerNorm(d_emb_q, eps=1e-6)

    # q.shape= torch.Size([16, 21, 74])
    # k.shape= torch.Size([16, 21, 35])
    # v.shape= torch.Size([16, 21, 35])
    def forward(self, q, k, v, mask1=None, mask2=None):
        ...
        # q.shape= torch.Size([16, 21, 1, 74])
        # k.shape= torch.Size([16, 21, 1, 74])
        # v.shape= torch.Size([16, 21, 1, 35])
        ...
        q_, k_ = q.view(sz_b, len_q, d_k), k.view(sz_b, len_k, d_k)#维度不变q_.shape= torch.Size([16, 21, 74]
        gate_ = self.fbp(q_, k_)#torch.Size([16, 21, 32])
        ...
        gate_ = (gate_sign + torch.abs(gate_sign)) / 2.0#正数转换为 1，负数和零转换为 0，
        ...
        q, k, v = q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2)#torch.Size([16, 1, 21, 74])
        ...
        result, attn = self.attention(q, k, v, mask1=mask1, mask2=mask2)#普通注意力torch.Size([16, 1, 21, 35])torch.Size([16, 1, 21, 21])
        ...
        result = result.transpose(1, 2).contiguous().view(sz_b, len_q, -1)#torch.Size([16, 21, 35])


        # b x l x (d_model)
        # result = self.dropout(self.fc(result.transpose(1, 2)).transpose(1, 2))
        result = self.dropout(self.fc(result))#torch.Size([16, 21, 74])
        ...
        result = result.masked_fill(torch.isnan(result), 0.0)#torch.Size([16, 21, 74])
